...
namespace OrderApi.Data.Test.Repository
{
	public class RepositoryTests : DatabaseTestBase
	{
		...
		public void AddAsync_WhenExceptionOccurs_ThrowsException()
		{
			...
			_testeeFake.Invoking(x => x.AddAsync(new Order())).Should().ThrowAsync<Exception>().WithMessage("entity could not be saved");
		}
		...
		public void UpdateAsync_WhenExceptionOccurs_ThrowsException()
		{
			...
			_testeeFake.Invoking(x => x.UpdateAsync(new Order())).Should().ThrowAsync<Exception>().WithMessage("entity could not be updated");
		}
		...
		public void UpdateRangeAsync_WhenExceptionOccurs_ThrowsException()
		{
			...
			_testeeFake.Invoking(x => x.UpdateRangeAsync(new List<Order>())).Should().ThrowAsync<Exception>().WithMessage("entities could not be updated");
		}

		[Theory]
		[InlineData("Updated FullName")]
		public async void UpdateOrderAsync_WhenOrdersIsNotNull_ShouldUpdateOrders(string newOrderFullName)
		{
			...
			(Context.Orders.All(x => x.CustomerFullName == newOrderFullName)).Should().BeTrue();
		}
	}
}
