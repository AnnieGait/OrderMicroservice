...
namespace OrderApi.Test.Controllers
{
	public class OrderControllerTests
	{
		...
		public OrderControllerTests()
		{
			...
			A.CallTo(() => _mediator.Send(A<GetPaidOrderQuery>._, A<CancellationToken>._)).Returns(orders.FindAll(x => x.OrderState == OrderState.Paid));
		}
		...
		public async void Order_WhenAnExceptionOccurs_ShouldReturnBadRequest(string exceptionMessage)
		{
			...
			var result = await _testee.Create(_orderModel);
		...
		public async void Pay_ShouldReturnOrder()
		{
			...
			(result.Result as StatusCodeResult)?.StatusCode.Should().Be((int)HttpStatusCode.OK);
			result.Value.Should().BeOfType<Order>();
			result.Value.OrderState.Should().Be(OrderState.Paid);
		}
	}
}
